...
const Products: React.FC = () => {
  ...
  // 按分後単価を計算する関数
  const calculateAllocatedCost = (product: any) => {
    // 関連データがない場合は purchase_cost を使用
    if (!product.store_purchases || !product.store_purchases.purchase_sessions) {
    ...
    // 店舗の直接コスト
    const storeDirectCost = (storePurchase.product_cost || 0) + 
    ...
    // セッション内の全店舗の総額を取得する必要があるが、
    // 簡略化のため、店舗の商品数で按分
    const itemCount = storePurchase.item_count || 1;
    
    // 共通コストを商品数で按分（簡略版）
    const allocatedCommonCost = commonCosts / itemCount;
    
    // 按分後単価 = (店舗直接コスト + 按分された共通コスト) / 商品数
    return Math.round((storeDirectCost + allocatedCommonCost) / itemCount);
  };
